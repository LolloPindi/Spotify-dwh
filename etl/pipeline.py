...
# Mappa: country_code -> [continent, subregion, primary_language, income_group, population, gdp_per_capita]
COUNTRY_METADATA = {
...
def run_etl():
    ...
    try:
        # 1. SVUOTAMENTO TABELLE DWH ( CASCADE )
        sep("1. PULIZIA E PREPARAZIONE DWH")
        with conn_dwh.cursor() as cur:
            for t in ["fact_chart_entry", "dim_tempo", "dim_paese", "dim_traccia", "dim_artista", "dim_album"]:
                cur.execute(f"TRUNCATE TABLE {t} CASCADE")
                print(f"  ✓  DWH: {t} svuotata")
        conn_dwh.commit()

        # 2. POPOLAMENTO DIM_TEMPO
        sep("2. POPOLAMENTO DIM_TEMPO")
        rec_dates = execute_query(conn_rec, "SELECT snapshot_date, year, month, quarter, week_of_year, day_of_week, season FROM snapshot_date")
        tempo_rows = []
        for row in rec_dates:
            dt = row[0]
            date_key = int(dt.strftime("%Y%m%d"))
            # dt, week (from week_of_year), month, quarter, year, season
        ...
        conn_dwh.commit()
        print(f"  ✓  Caricate {len(tempo_rows):,} righe in dim_tempo")

        # 3. POPOLAMENTO DIM_PAESE (CON ARRICCHIMENTO GEOPOLITICO)
        ...
        for code, name in rec_countries:
            ...
            # code, name, continent, subregion, language, income_group, population, gdp_per_capita
        ...
        conn_dwh.commit()
        print(f"  ✓  Caricate {len(paese_rows):,} righe in dim_paese con arricchimento.")

        # 4. POPOLAMENTO DIM_TRACCIA (CON DISCRETIZZAZIONE AUDIO FEATURES + GENERE LAST.FM)
        sep("4. POPOLAMENTO DIM_TRACCIA")
        ...
        traccia_rows = []
        for r in rec_tracks:
            # spotify_id, name, duration_ms, is_explicit, danceability, energy, valence, lastfm_genre
            danceability = r[4]
            energy = r[5]
            valence = r[6]
            genre = r[7]  # Last.fm genre (can be None)
            mood_band = discretize_feature(valence)
            energy_band = discretize_feature(energy)
            valence_band = mood_band  # Alias per consistenza DFM
            traccia_rows.append((
                r[0], r[1], r[2], r[3],
            ...
            psycopg2.extras.execute_values(
                ...
                """INSERT INTO dim_traccia
                   (spotify_id, name, duration_ms, is_explicit,
                    danceability, energy, valence,
                    mood_band, energy_band, valence_band, genre)
                   VALUES %s""",
                traccia_rows
            )
        conn_dwh.commit()
        genre_filled = sum(1 for r in traccia_rows if r[10] is not None)
        print(f"  ✓  Caricate {len(traccia_rows):,} righe in dim_traccia")
        print(f"       Genere Last.fm disponibile: {genre_filled:,} / {len(traccia_rows):,} ({100*genre_filled/max(len(traccia_rows),1):.1f}%)")

        # 5. POPOLAMENTO DIM_ARTISTA (CON GENERE LAST.FM)
        sep("5. POPOLAMENTO DIM_ARTISTA")
        rec_artists = execute_query(
            conn_rec,
            "SELECT artist_id, name, lastfm_genre FROM artist"
        )
        with conn_dwh.cursor() as cur:
            psycopg2.extras.execute_values(
                cur,
                "INSERT INTO dim_artista (artist_id, name, genre) VALUES %s",
                [(r[0], r[1], r[2]) for r in rec_artists]
            )
        conn_dwh.commit()
        genre_filled_a = sum(1 for r in rec_artists if r[2] is not None)
        print(f"  ✓  Caricate {len(rec_artists):,} righe in dim_artista")
        print(f"       Genere Last.fm disponibile: {genre_filled_a:,} / {len(rec_artists):,} ({100*genre_filled_a/max(len(rec_artists),1):.1f}%)")

        # 6. POPOLAMENTO DIM_ALBUM
        sep("6. POPOLAMENTO DIM_ALBUM")
        ...
        conn_dwh.commit()
        print(f"  ✓  Caricate {len(album_rows):,} righe in dim_album")

        # ── In-Memory Lookup Dicts per Surrogate Keys ────────────────────────
        sep("CARICAMENTO MAPPE SURROGATE KEYS IN MEMORIA")
        ...
        # ── Pre-calcolo 'n_countries_charted' in Python con bisect ─────────────
        sep("PRE-CALCOLO CUMULATIVO COUNTRY ENTRIES")
        print("  Eseguo query delle prime entry per paese...")
        ...
        print("  ✓  Mappa cumulativa pre-calcolata.")
        ...
        print("  ✓  Relazioni track-artist caricate.")

        # 7. POPOLAMENTO FACT_CHART_ENTRY
        sep("7. POPOLAMENTO FACT_CHART_ENTRY (2M+ righe × split N:M)")
        # Estrazione con Window Functions per days_on_chart e peak_rank
        rec_fact_query = """
            SELECT 
                ce.spotify_id,
                ce.country_code,
                ...
                ce.daily_movement,
                ce.weekly_movement,
                -- Window function per conteggio progressivo giorni in classifica
                ROW_NUMBER() OVER (PARTITION BY ce.spotify_id, ce.country_code ORDER BY ce.snapshot_date) AS days_on_chart,
                -- Window function per miglior rank raggiunto
                MIN(ce.daily_rank) OVER (PARTITION BY ce.spotify_id, ce.country_code ORDER BY ce.snapshot_date) AS peak_rank,
                t.album_id
            FROM chart_entry ce
        ...
        cur_rec = conn_rec_stream.cursor(name="stream_fact") # Server-side cursor per non saturare la RAM
        ...
        print("  Inizio streaming dei fatti dal Reconciled DB...")
        while True:
            ...
            total_read += len(rows)
            for r in rows:
                sid, ccode, sdt, daily_rank, popularity, daily_mov, weekly_mov, days_on_chart, peak_rank, album_id = r
                ...
                if not country_key or not track_key or not album_key:
                    # Salta se mancano le chiavi dimensionali
                    continue

                # Calcolo n_countries_charted usando bisect_right
                entry_dates = track_min_dates.get(sid, [])
                n_countries_charted = bisect.bisect_right(entry_dates, sdt)

                # N:M Split — una riga dei fatti per ogni artista del brano
                artists_for_track = track_artists.get(sid, [])
                for artist_id in artists_for_track:
                    artist_key = dwh_artists.get(artist_id)
                    if not artist_key:
                        continue
        ...
        print(f"  ✓  Fatto: {total_inserted:,} righe totali caricate in fact_chart_entry (righe lette: {total_read:,})")

        # ── Verifica coerenza e firma cubo ───────────────────────────────────
        sep("8. VERIFICA INTEGRITÀ E CALCOLO FIRMA CUBO (DWH)")
        dwh_counts = {}
        for t in ["dim_tempo", "dim_paese", "dim_traccia", "dim_artista", "dim_album", "fact_chart_entry"]:
            n = execute_query(conn_dwh, f"SELECT COUNT(*) FROM {t}")[0][0]
            dwh_counts[t] = n
            print(f"  DWH: {t:<20} {n:>10,} righe")

        # Calcolo dell'hash/firma del cubo per caching narrativo
        # Un hash basato sui totali del DWH e sulla somma di daily_rank della fact
        fact_sum = execute_query(conn_dwh, "SELECT SUM(daily_rank), SUM(popularity) FROM fact_chart_entry")[0]
        sig_str = f"{dwh_counts['fact_chart_entry']}|{fact_sum[0]}|{fact_sum[1]}"
        cube_sig = hashlib.md5(sig_str.encode()).hexdigest()
        print(f"\n  Firma del Cubo DWH: {cube_sig} (per layer AI cache)")

        sep("ETL COMPLETATO CON SUCCESSO!")

    except Exception as e:
        conn_dwh.rollback()
        print(f"\n  ✗ Errore fatale durante ETL: {e}", file=sys.stderr)
        raise
    finally:
        conn_rec.close()
        conn_dwh.close()
